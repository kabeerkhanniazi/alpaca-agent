...
def make_position_manager_node(broker, config: AgentConfig):
    """Build the position-manager node."""

    def position_manager_node(state: OptionsAgentState) -> dict[str, Any]:
        spot_prices = {}
        if state.get("ticker") and state.get("spot"):
            spot_prices[state["ticker"]] = state["spot"]

        try:
            portfolio = build_portfolio_state(broker, config, spot_prices)
        except Exception as exc:  # noqa: BLE001
            logger.error("Could not build portfolio state: %s", exc)
            return {
                "halted": True,
                "halt_reason": f"Portfolio state unavailable: {exc}",
                "errors": [*state.get("errors", []), str(exc)],
            }

        return {"portfolio": portfolio, "exits": decide_exits(portfolio, config)}
