...
def make_risk_gate_node(config: AgentConfig):
    """Build the risk-gate node. Pure computation — no broker, no LLM."""

    def risk_gate_node(state: OptionsAgentState) -> dict[str, Any]:
        if state.get("halted"):
            return {}
        ...
        if not spread:
            return {
                "approved": False,
                "gate_reason": "No spread was proposed for this cycle.",
                "gate_checks": [],
            }

        portfolio = dict(state.get("portfolio") or {})
        portfolio.setdefault("spot", state.get("spot"))

        result = risk_gate_check(spread, portfolio, config)
        return {
            "approved": result.approved,
            "gate_reason": result.reason,
            "gate_checks": [c.to_dict() for c in result.checks],
