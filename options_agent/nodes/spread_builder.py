...
def make_spread_builder_node(config: AgentConfig):
    """Build the spread-builder node. Pure computation — no broker needed."""

    def spread_builder_node(state: OptionsAgentState) -> dict[str, Any]:
        if state.get("halted"):
            return {}
        ...
        if nav <= 0:
            return {
                "halted": True,
                "halt_reason": "Account NAV unavailable; cannot size positions.",
            }
        ...
        if not spreads:
            return {
                ...
                "halted": True,
                "halt_reason": (
                    f"No {config.spread_widths[0]:.0f}-{config.spread_widths[1]:.0f} wide bull put "
                    f"spread on {state['ticker']} produced a positive credit within the "
                    f"{config.max_loss_pct:.0%}-of-NAV loss budget."
                ),
            }

        # The gate evaluates the best-ranked spread. The rest stay in state so
        # the journal and dashboard can show what else was on the table.
        return {"spreads": spreads, "selected_spread": spreads[0]}

    return spread_builder_node
