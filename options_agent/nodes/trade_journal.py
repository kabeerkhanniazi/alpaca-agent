...
def make_journal_node(journal: TradeJournal):
    """Build the terminal journal node.

    Runs on every path — approved, rejected, or halted — so no cycle passes
    through the graph without leaving a record of what happened and why.
    """

    def journal_node(state) -> dict[str, Any]:
        run_id = state.get("run_id", "")
        ticker = state.get("ticker", "")

        if state.get("market_context"):
            journal.log_analysis(run_id, ticker, state["market_context"])

        if state.get("halted"):
            journal.log_skip(run_id, ticker, state.get("halt_reason", "halted"))
            return {}

        if state.get("spreads"):
            journal.log_spread_candidates(run_id, ticker, state["spreads"])
        ...
        journal.log_gate_decision(
            ...
            checks=state.get("gate_checks", []),
            contracts=int(state.get("contracts") or 0),
        )

        if state.get("execution"):
            journal.log_execution(run_id, ticker, state.get("selected_spread") or {}, state["execution"])
